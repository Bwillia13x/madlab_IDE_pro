...
(() => {
  ...
  function validate() {
    ...
    if (!isFinite(vals.fcf0))
      return { ok: false, focusId: 'fcf0', message: 'FCF0 must be a number' };
    ...
    if (!(vals.horizon > 0))
      return { ok: false, focusId: 'horizon', message: 'Horizon must be > 0' };
  ...
  copyBtn.addEventListener('click', async () => {
    try {
      const data = JSON.stringify(window.__madlab_metrics || {}, null, 2);
      ...
      hint.textContent =
        'Unable to copy metrics. Please open DevTools > Console and copy manually.';
    }
  });
  app.append(formRow1, formRow2, calcBtn, copyBtn, hint, result);
  window.__madlab_metrics = { initialRenderMs: performance.now() - t0 };
  console.log('[MadLab] initialRenderMs', window.__madlab_metrics.initialRenderMs.toFixed(2));
  let recomputeSamples = [];
  window.addEventListener('message', (event) => {
    ...
    switch (message.type) {
      ...
      default:
        console.warn('[MadLab] Unhandled message type', message.type);
    }
  });
  setTimeout(() => {
    const runs = 50;
