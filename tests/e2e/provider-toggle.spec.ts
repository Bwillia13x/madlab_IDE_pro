...
test.describe('Data Provider Toggle', () => {
  test('remains on Mock when extension bridge is unavailable', async ({ page }) => {
    ...
    await page.getByTestId('demo-banner').locator('[aria-label="Dismiss demo mode banner"]').click({ trial: true }).catch(() => {});
  ...
  test('switches to Extension when bridge is available', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      (window as any).madlabBridge = {
        request: async (type: string, payload?: any) => {
          ...
          if (type === 'data:vol') {
            return { symbol: 'AAPL', underlyingPrice: 100, points: [], timestamp: new Date().toISOString() }
          }
    ...
    await page.getByTestId('demo-banner').locator('[aria-label="Dismiss demo mode banner"]').click().catch(() => {});

    // Click provider toggle to sync label, then dispatch event for store
    await providerBtn.click({ force: true })
    await page.locator('html').evaluate((el) => el.setAttribute('data-extension-bridge', 'true')).catch(() => {})
