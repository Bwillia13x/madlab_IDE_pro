...
import { calculateReturns, historicalVaR, expectedShortfall, cornishFisherVaR, bootstrapVaREs } from '@/lib/quant/risk';
...
function VarEsComponent({ config, onConfigChange }: WidgetProps) {
  ...
  const [metrics, setMetrics] = useState(() => ({ varHist: NaN, esHist: NaN, varCF: NaN, ci: { varHist: [NaN, NaN] as [number, number], esHist: [NaN, NaN] as [number, number], varCF: [NaN, NaN] as [number, number] } }))
  const workerRef = useRef<Worker | null>(null)

  useEffect(() => {
    let cancelled = false
    if (!returns || returns.length === 0) {
      setMetrics({ varHist: NaN, esHist: NaN, varCF: NaN, ci: { varHist: [NaN, NaN], esHist: [NaN, NaN], varCF: [NaN, NaN] } })
      return
    }
    ...
    try {
      ...
      const handleMessage = (e: MessageEvent) => {
          if (cancelled) return
        const msg = e.data as { type: 'result' | 'error'; data?: { varHist: number; esHist: number; ci: { varHist: [number, number]; esHist: [number, number] } } }
        if (msg?.type === 'result' && msg.data) {
          ...
          const fallback = bootstrapVaREs(returns, cfg.confidence, { samples: 500, ci: [0.05, 0.95] })
            setMetrics({
        ...
        worker.postMessage({ type: 'bootstrap', returns, confidence: cfg.confidence, samples: 500 })
        return () => {
          cancelled = true
    ...
    <div className="h-full space-y-3">
      <div className="grid grid-cols-3 gap-2 text-xs">
        <label className="flex flex-col gap-1">
          <span className="text-muted-foreground inline-flex items-center gap-1">Confidence
            <Popover>
              <PopoverTrigger asChild>
                <button type="button" aria-label="What is VaR?" aria-haspopup="dialog" aria-controls="vares-help-var" className="h-4 w-4 inline-flex items-center justify-center rounded hover:bg-accent">
                  <HelpCircle className="h-3 w-3" />
                </button>
              </PopoverTrigger>
              <PopoverContent id="vares-help-var" className="w-64 text-xs" side="bottom">
                <div className="font-medium mb-1">VaR & ES</div>
                <p className="text-muted-foreground">VaR is potential loss at a confidence level. ES is average loss beyond VaR (tail risk).</p>
              </PopoverContent>
            </Popover>
          </span>
          <input type="number" step={0.01} value={cfg.confidence} onChange={(e) => updateConfig({ confidence: Math.max(0.5, Math.min(0.999, Number(e.target.value) || 0.95)) })} className="bg-background border border-border rounded px-2 py-1" />
        </label>
        <label className="flex flex-col gap-1">
          <span className="text-muted-foreground">Window</span>
          <input type="number" step={1} value={cfg.window} onChange={(e) => updateConfig({ window: Math.max(20, Math.floor(Number(e.target.value) || 252)) })} className="bg-background border border-border rounded px-2 py-1" />
        </label>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="p-3 bg-card rounded border border-border">
          ...
          <p className="text-[10px] text-muted-foreground">CI: {fmtPct(metrics.ci.varHist[0])} — {fmtPct(metrics.ci.varHist[1])}</p>
        </div>
        <div className="p-3 bg-card rounded border border-border">
          <div className="flex items-center gap-2 mb-2">
          ...
          <p className="text-[10px] text-muted-foreground">CI: {fmtPct(metrics.ci.esHist[0])} — {fmtPct(metrics.ci.esHist[1])}</p>
        </div>
        <div className="p-3 bg-card rounded border border-border">
          <div className="flex items-center gap-2 mb-2">
          ...
          <p className="text-[10px] text-muted-foreground">CI: {fmtPct(metrics.ci.varCF[0])} — {fmtPct(metrics.ci.varCF[1])}</p>
        </div>
      </div>

      <div className="text-xs text-muted-foreground text-center">
        1-Day Risk Metrics | Symbol: {cfg.symbol?.toUpperCase() || 'ACME'} | n={returns?.length ?? 0}
      </div>
      <div className="flex justify-end">
        <button className="text-xs underline text-muted-foreground" onClick={() => setAssessmentOpen(true)}>Quick VaR Check</button>
      </div>
      <Dialog open={assessmentOpen} onOpenChange={setAssessmentOpen}>
        <DialogContent aria-label="VaR knowledge check">
          ...
          <VarAssessment onClose={(pass) => {
            try { localStorage.setItem('madlab_assess_var', pass ? 'pass' : 'fail'); } catch {}
            analytics.track('assessment_completed', { widget: 'var-es', pass }, 'feature_usage');
            if (pass) {
              try { require('@/lib/store').useWorkspaceStore.getState().celebrate?.('Great job!'); } catch {}
            }
...
export function VarEsLegacy({ widget: _widget, symbol }: Readonly<VarEsProps>) {
  ...
  const metrics = useMemo(() => {
    if (!returns || returns.length === 0) {
      return { varHist: NaN, esHist: NaN, varCF: NaN, ci: { varHist: [NaN, NaN], esHist: [NaN, NaN], varCF: [NaN, NaN] } } as any;
    }
    ...
    <div className="h-full space-y-3">
      <div className="grid grid-cols-3 gap-2 text-xs">
        <label className="flex flex-col gap-1">
          ...
          <input type="number" step={0.01} value={confidence} onChange={(e) => setConfidence(Math.max(0.5, Math.min(0.999, Number(e.target.value) || 0.95)))} className="bg-background border border-border rounded px-2 py-1" />
        </label>
        <label className="flex flex-col gap-1">
          <span className="text-muted-foreground">Window</span>
          <input type="number" step={1} value={window} onChange={(e) => setWindow(Math.max(20, Math.floor(Number(e.target.value) || 252)))} className="bg-background border border-border rounded px-2 py-1" />
        </label>
        <div className="flex items-end text-muted-foreground">1-Day horizon</div>
      </div>
...
export const VarEsDefinition: WidgetDefinition = {
  meta: {
    type: 'var-es',
    name: 'VaR / ES',
    description: 'Historical VaR, Expected Shortfall, and Cornish-Fisher VaR with bootstrap confidence intervals',
    category: 'risk',
    version: '1.0.0',
    difficulty: 'advanced',
    edu: { help: 'VaR estimates potential loss at a confidence level; ES estimates average loss beyond VaR.' },
  ...
  return (
    <div className="space-y-3 text-sm">
      <div>
        ...
        <select className="mt-1 w-full bg-background border border-border rounded px-2 py-1" value={ans.q1 || ''} onChange={(e) => setAns(a => ({ ...a, q1: e.target.value }))}>
      ...
      <div>
        ...
        <select className="mt-1 w-full bg-background border border-border rounded px-2 py-1" value={ans.q2 || ''} onChange={(e) => setAns(a => ({ ...a, q2: e.target.value }))}>
          <option value="">Select…</option>
      ...
      <div className="flex justify-end">
        <button className="h-7 px-3 rounded border border-border text-xs" onClick={() => onClose(ans.q1 === 'threshold' && ans.q2 === 'tailavg')}>Submit</button>
      </div>
    </div>
  );
}
