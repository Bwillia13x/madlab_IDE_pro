...
function isBridgeAvailable(): boolean {
  return typeof window !== 'undefined' && !!window.madlabBridge && typeof window.madlabBridge.request === 'function';
}
...
export const extensionProvider: DataProvider = {
  ...
  async getPrices(symbol: string, range: PriceRange = '6M'): Promise<PricePoint[]> {
    ...
    const res = await req<Array<any>>('data:prices', { symbol, range });
    const mapped = (Array.isArray(res) ? res : []).map((p) => ({
      date: new Date((p as any).date ?? p?.date),
      open: Number((p as any).open ?? 0),
      high: Number((p as any).high ?? 0),
      low: Number((p as any).low ?? 0),
      close: Number((p as any).close ?? 0),
  ...
  async getKpis(symbol: string): Promise<KpiData> {
    if (!isBridgeAvailable()) throw new Error('Extension bridge unavailable');
    const req = window.madlabBridge!.request! as <T>(m: string, p?: unknown) => Promise<T>;
    const k = await req<any>('data:kpis', { symbol });
    const parsed = zKpiData.safeParse({ ...k, timestamp: new Date((k as any)?.timestamp ?? Date.now()) });
    if (!parsed.success) {
  ...
  async getVolSurface(symbol: string): Promise<VolSurface> {
    ...
    const mapped = {
      ...
      points: (Array.isArray((v as Record<string, unknown>)?.points) ? (v as Record<string, unknown>).points as unknown[] : []).map((pt: unknown) => ({
        strike: Number((pt as Record<string, unknown>)?.strike ?? 0),
        expiry: new Date((pt as Record<string, unknown>)?.expiry as string | number | Date ?? Date.now()),
        impliedVol: Number((pt as Record<string, unknown>)?.impliedVol ?? 0),
      })),
      timestamp: new Date((v as Record<string, unknown>)?.timestamp as string | number | Date ?? Date.now()),
    };
  ...
  async getFinancials(symbol: string): Promise<any> {
    if (!isBridgeAvailable()) throw new Error('Extension bridge unavailable');
    const req = window.madlabBridge!.request! as <T>(m: string, p?: unknown) => Promise<T>;
    const f = await req<any>('data:financials', { symbol });
    const mapped = { ...f, timestamp: new Date((f as any)?.timestamp ?? Date.now()) };
