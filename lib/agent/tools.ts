...
export function add_widget(args: {
  type: string;
  title?: string;
...
export function fetch_prices(args: { symbol: string; range?: PriceRange }): ToolResult {
  ...
  try {
    ...
    if (!provider)
      return { ok: false, message: 'No data provider' };
...
export function runToolCall(tc: ToolCall): ToolResult {
  ...
  const isAddWidgetArgs = (
    a: unknown
  ): a is { type: string; title?: string; props?: Record<string, unknown> } =>
  ...
  switch (tc.name) {
    case 'add_sheet':
      return isAddSheetArgs(tc.args)
        ? add_sheet(tc.args)
        : { ok: false, message: 'Invalid args for add_sheet' };
    case 'add_widget':
      return isAddWidgetArgs(tc.args)
        ? add_widget(tc.args)
        : { ok: false, message: 'Invalid args for add_widget' };
    case 'select_widget':
      return isSelectArgs(tc.args)
        ? select_widget(tc.args)
        : { ok: false, message: 'Invalid args for select_widget' };
    case 'rename_sheet':
      return isRenameArgs(tc.args)
        ? rename_sheet(tc.args)
        : { ok: false, message: 'Invalid args for rename_sheet' };
    case 'duplicate_widget':
      return isDuplicateArgs(tc.args)
        ? duplicate_widget(tc.args)
        : { ok: false, message: 'Invalid args for duplicate_widget' };
    case 'open_inspector':
      return isInspectorArgs(tc.args)
        ? open_inspector(tc.args)
        : { ok: false, message: 'Invalid args for open_inspector' };
    case 'fetch_prices':
      return isRecord(tc.args) && typeof tc.args['symbol'] === 'string'
        ? fetch_prices(tc.args as { symbol: string; range?: string })
        : { ok: false, message: 'Invalid args for fetch_prices' };
    case 'fetch_kpis':
      return isRecord(tc.args) && typeof tc.args['symbol'] === 'string'
        ? fetch_kpis(tc.args as { symbol: string })
        : { ok: false, message: 'Invalid args for fetch_kpis' };
    case 'remove_widget':
